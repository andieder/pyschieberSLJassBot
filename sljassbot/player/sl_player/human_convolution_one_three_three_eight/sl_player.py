...
from sljassbot.player.sl_player.human_convolution_one_three_three_eight.input_handler import InputHandler, card_to_index, index_to_card
from sljassbot.player.sl_player.human_convolution_one_three_three_eight.model import build_model

logger = logging.getLogger(__name__)


class SLPlayer1338hc(BasePlayer):
    max_points = 55
    min_points = -55
    ...
    def act(self, input_state):
        ...
        act_values = self.game_model.predict(self.create_model_input_state(input_state))
        return act_values, np.argsort(act_values[0])[::-1]

    def create_model_input_state(self, input_state):
        dict_input_state = {}
        for i in range(0, self.input_handler_game_network.amount_card_sets):
            dict_input_state['input_{}'.format(i + 1)] = np.expand_dims(
                input_state[i * self.input_handler_game_network.nr_cards:
                            i * self.input_handler_game_network.nr_cards + self.input_handler_game_network.nr_cards],
                axis=0)
        dict_input_state['input_38'] = np.expand_dims(
            input_state[self.input_handler_game_network.pos_trumpf:
                        self.input_handler_game_network.pos_trumpf + self.input_handler_game_network.nr_trumpf_modes],
            axis=0)
        return dict_input_state
